	...
	private void doStartupChecks() throws XBeeException {
		...
		try {				
			...
			if (ap.getValue()[0] != 2) {
				...
				log.log("Attempting to set AP to 2");
				ap = this.sendAtCommand(new AtCommand("AP", 2));
				
				if (ap.isOk()) {
					log.log("Successfully set AP mode to 2.  This setting will not persist a power cycle without the WR (write) command");	
				} else {
					throw new XBeeException("Attempt to set AP=2 failed");
				}
			} else {
				log.log("Radio is in correct AP mode (AP=2)");
			}
			...
			if (ap.isOk()) {
				...
				case 0x17:
					log.log("XBee radio is Series 1");
					break;
				case 0x18:
					log.log("XBee radio is Series 1 Pro");
					break;
				case 0x19:
					log.log("XBee radio is Series 2");
					break;
				case 0x1a:
					log.log("XBee radio is Series 2 Pro");
					break;
				case 0x1d:
					log.log("XBee radio is Series Pro 868");
					break;
				default:
